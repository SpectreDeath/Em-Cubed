...
class PluginManager:
    ...
    def _load_plugins(self):
        ...
        eager_builtin, lazy_builtin, entry_point, directory = self.discovery.discover_all_plugins()
            ...
            if surface_class is not None:
                try:
                    ...
                    if issubclass(surface_class, SurfacePlugin):
                        ...
                        logger.warning(
                            "Discovered class is not a SurfacePlugin", surface=name, class_name=surface_class.__name__
                        )
        ...
        for name, surface_class in lazy_builtin.items():
            if surface_class is not None:
                ...
                if issubclass(surface_class, SurfacePlugin):
                    ...
                    logger.warning(
                        "Discovered lazy class is not a SurfacePlugin", surface=name, class_name=surface_class.__name__
                    )
        ...
        for name, surface_class in entry_point.items():
            try:
                ...
                if issubclass(surface_class, SurfacePlugin):
                    ...
                    logger.warning(
                        "Entry point class is not a SurfacePlugin", entry_point=name, class_name=surface_class.__name__
                    )
            except Exception as e:
                logger.warning("Failed to load entry point plugin", entry_point=name, error=str(e))
        ...
        for name, surface_class in directory.items():
            try:
                ...
                if issubclass(surface_class, SurfacePlugin):
                    ...
                    logger.warning(
                        "Directory class is not a SurfacePlugin", plugin_name=name, class_name=surface_class.__name__
                    )
            except Exception as e:
                logger.warning("Failed to load directory plugin", plugin_name=name, error=str(e))
        ...
        if name in self._lazy_classes:
            try:
                ...
                if issubclass(surface_class, SurfacePlugin):
                    ...
                    logger.warning(
                        "Lazily loaded class is not a SurfacePlugin", surface=name, class_name=surface_class.__name__
                    )
                    return None
            except Exception as e:
