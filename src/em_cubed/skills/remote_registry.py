...
class RemoteSkillRegistry:
    ...
    def add_registry(
        ...
        self._registries[name] = {
            ...
            "headers": {"Authorization": f"Bearer {token}" if token else None, "User-Agent": "Em-Cubed/0.5.0"},
        }
    ...
    def _save_cached_registry(self, registry_name: str, data: List[Dict[str, Any]]):
        ...
        try:
            ...
            self.logger.debug("Saved registry data to cache", registry=registry_name, count=len(data))
        except Exception as e:
    ...
    def _fetch_remote_registry(self, registry_name: str) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            response = requests.get(url, headers=config["headers"], verify=config["verify_ssl"], timeout=30)
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, dict) and "skills" in data:
                    ...
                    self.logger.error(
                        "Unexpected registry response format", registry=registry_name, format=type(data).__name__
                    )
                    return None
            ...
            self.logger.error("Network error fetching remote registry", registry=registry_name, error=str(e))
            return None
        except Exception as e:
        ...
        try:
            ...
            for field in required_fields:
                if field not in skill_data:
                    self.logger.warning(
                        "Missing required field in remote skill", field=field, skill_id=skill_data.get("skill_id")
                    )
                    return None
            ...
            self.logger.warning(
                "Failed to convert remote skill to metadata", skill_id=skill_data.get("skill_id"), error=str(e)
            )
            return None
    ...
    def sync_registry(self, registry_name: str, force: bool = False) -> bool:
        ...
        if skills_data is None or force:
            ...
            if skills_data is None:
                ...
                skills_data = self._load_cached_registry(registry_name)
                if skills_data is None:
                    self.logger.error("Failed to fetch or load registry data", registry=registry_name)
                    return False
            else:
        ...
        for skill_data in skills_data:
            try:
                ...
                if existing_skill is None:
                    ...
                    imported_count += 1
                    self.logger.info(
                        "Imported new skill from remote registry", skill_id=skill_id, registry=registry_name
                    )
                else:
                    ...
                    if remote_version != local_version:
                        ...
                        updated_count += 1
                        self.logger.info(
                            "Updated skill from remote registry",
                            skill_id=skill_id,
                ...
                self.logger.warning(
                    "Failed to process skill from remote registry", skill_id=skill_data.get("skill_id"), error=str(e)
                )
                ...
                for skill_data in cache:
                    ...
                    if query.lower() in skill_name or query.lower() in skill_desc:
                        try:
                            ...
                            self.logger.warning("Failed to parse remote skill", registry=name, error=str(e))
