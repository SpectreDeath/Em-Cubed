...
class TypeConverter:
    ...
    def convert_to_surface(self, value: Any, surface: str, type_hint: str | None = None) -> Any:
        ...
        if not type_def:
            self.logger.warning("Unknown type hint, returning value as-is", type_hint=type_hint)
            return value
        ...
        try:
            if surface == "python":
                ...
                self.logger.warning("Unknown surface, returning value as-is", surface=surface)
                return value
        except Exception as e:
            self.logger.error(
                "Failed to convert value to surface", value=value, surface=surface, type_hint=type_hint, error=str(e)
            )
    ...
    def convert_from_surface(self, value: Any, surface: str, type_hint: str | None = None) -> Any:
        ...
        try:
            ...
            self.logger.error(
                "Failed to convert value from surface", value=value, surface=surface, type_hint=type_hint, error=str(e)
            )
            # Return original value as fallback
            return value
