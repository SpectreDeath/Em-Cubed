...
async def test_runtime_schema_validation():
    ...
    input_schema = InputOutputSchema(type="object", properties={"value": {"type": "integer"}}, required=["value"])
    output_schema = InputOutputSchema(type="object", properties={"result": {"type": "string"}}, required=["result"])
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch.object(executor, "_load_skill_code", return_value={"python": "pass"}):
            ...
            req_invalid_input = SkillExecutionRequest(skill_id="test/validator", input_data={"value": "not-an-integer"})
            ...
            req_valid_input = SkillExecutionRequest(skill_id="test/validator", input_data={"value": 42})
...
def test_boundary_type_coercion():
    ...
    data = {"id": "789", "rate": 1.25, "flag": "no", "tags": [1, 2, False], "ignored": "should-stay-same"}
...
async def test_logic_surfaces_caching():
    ...
    with (
        ...
        patch("tempfile.mkstemp", return_value=(0, "/fake/path")),
    ):
    ...
    with (
        patch.object(datalog_surface, "_validate_code", return_value=None),
        patch.object(datalog_surface, "_check_availability", return_value=True),
    ):
        # Runs once
