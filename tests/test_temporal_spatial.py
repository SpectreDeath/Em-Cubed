...
def test_time_interval_validity():
    ...
    t_mid = datetime(2026, 3, 15, tzinfo=UTC)
    t_out = datetime(2026, 8, 1, tzinfo=UTC)
...
def test_haversine_distance_and_spatial_proximity():
    ...
    t_triple = TemporalSpatialTriple(
        ...
        interval=TimeInterval(start_time=datetime(2026, 1, 1, tzinfo=UTC)),
        location=loc2,
    )
...
def test_temporal_snapshot_query():
    ...
    snapshot_feb = TemporalSnapshotQueryEngine.snapshot_at(timeline, datetime(2026, 2, 1, tzinfo=UTC))
    assert len(snapshot_feb) == 1
    assert snapshot_feb[0].object == "InTransit"

    # Query snapshot at May 2026 (late)
    snapshot_may = TemporalSnapshotQueryEngine.snapshot_at(timeline, datetime(2026, 5, 1, tzinfo=UTC))
