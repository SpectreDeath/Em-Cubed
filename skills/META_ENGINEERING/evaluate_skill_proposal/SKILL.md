---
name: evaluate_skill_proposal
domain: META_ENGINEERING
version: "1.0.0"
surfaces: [prolog, python]
description: Evaluates a proposed skill name and description to map its professional taxonomy, required reference materials, meta-skills, and subskill dependencies using the Prolog ontology ground truth.
...
Intake a raw skill proposal (name + description) and produce a structured blueprint that maps the proposal to validated professions, canonical domains, authoritative reference materials, hierarchical subskill ancestry, and required meta-skill wrappers.

# Description

Acts as the ingestion gateway between the human / agent proposing a new skill and the framework's authoritative Prolog ontology (`skills/META_ENGINEERING/skill-taxonomy/skill_taxonomy.pl`).

The extraction engine never guesses — every output field is grounded in a structured Prolog relation. Where Prolog is unavailable at runtime, a deterministic built-in fallback ensures the same output shape.

# Surfaces

- **Prolog (primary)**: Queries the ontology for `profession_domain/2`, `recommend_materials/2`, `resolve_skill_ancestry/2`, `infer_professions_from_keywords/2`, `belongs_to_profession/2`, `requires_meta_wrapper/2`.
- **Python (orchestration)**: Normalises input, dispatches queries, assembles the blueprint dict, and serialises to JSON.

# Handler

## `evaluate_skill_proposal/handler.py`
...
class SkillExtractionEngine:
    ...
    def _infer_professions(self, tokens: List[str]) -> List[str]:
        ...
        # Fallback: deterministic keyword scan
        matched: List[str] = []
        seen: set = set()
    ...
    def _resolve_domains(self, professions: List[str]) -> List[str]:
        ...
        # Fallback: compact hard-coded domain spread
        _FALLBACK_DOMAINS = {
    ...
    def _infer_subskills(
        ...
        # Fallback: match description keywords to subskill trees
...
def evaluate_skill_proposal(
    ...
    *,
    ...
    ----------
    proposed_name:
        Raw working name of the proposed skill.
    proposed_description:
        Free-text description of what the skill should do.
    prolog_engine:
        Optional pre-initialised PrologTaxonomyEngine.

    Returns
    -------
    dict
        Blueprint with professions, core_domains, core_references,
        meta_skills_required, suggested_subskills, skill_ancestry, confidence.

    Examples
    --------
    >>> bp = evaluate_skill_proposal(
    ...     "execute_monte_carlo_walk",
    ...     "Simulate Markov chain trajectories and collect histogram data",
    ... )
    >>> bp["professions"]
    ['data_scientist']
    >>> bp["core_domains"]
...
The extraction engine is designed to be called in two ways:

1. **Direct function call** (preferred):

   ```python
   from em_cubed.skills.META_ENGINEERING.evaluate_skill_proposal.handler import evaluate_skill_proposal
   blueprint = evaluate_skill_proposal(
       proposed_name="execute_monte_carlo_walk",
       proposed_description="Simulate Markov chain trajectories and collect histogram data",
   )
   ```

2. **Via Prolog surface** (external caller queries the ontology directly):
   ...
   ?- recommend_materials(statistician, Ref).
   Ref = "Wackerly - Mathematical Statistics with Applications" ;
   Ref = "Ross - Introduction to Probability Models" ;
   false.

   ?- resolve_skill_ancestry(execute_monte_carlo_walk, Ancestry).
   Ancestry = ancestry(execute_monte_carlo_walk, data_scientist, stochastic_processes, root).

   ?- infer_professions_from_keywords([markov, stochastic, simulation], P).
   P = [data_scientist, statistician, operations_researcher].

   ?- get_domain_skills(stochastic_processes, Skills).
   Skills = [evaluate_stationarity, generate_transition_matrix, execute_monte_carlo_walk,
             calculate_pagerank_vector, compile_simulation_histogram,
             n_gram_token_predictor, assert_context_gate].
   ```
